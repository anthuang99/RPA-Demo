...
//	public static Integer errorIndex=0;
	private static final Log log=new Log(Assertion.class);
	...
	{
		WebDriver driver=TestBaseCase.getWebDriver();
		ScreenShot screenShot=new ScreenShot(getWebDriver());
		//设置截图名字
		Date nowDate=new Date();
		screenShot.setscreenName(Assertion.formatDate(nowDate));
		screenShot.takeScreenshot();
		//Assertion.getAssertInfolList().add("&lt;a href=\"snapshot/"+Assertion.formatDate(nowDate)+".jpg\" &gt;&lt;img height=\"100\" width=\"100\" src=\"snapshot\\"+Assertion.formatDate(nowDate)+".jpg\"&gt;&lt;/img&gt;&lt;/a&gt;&lt;br/&gt;"+"&lt;a href=\"snapshot\\"+Assertion.formatDate(nowDate)+".jpg\" &gt;点击查看大图&lt;/a&gt;\n");
	...
	public  static void VerityCationString(String actual,String exceptStr)
	{
		...
		try {
			...
			AssertPassLog();
			getAssertInfolList().add(verityStr+":pass");

		} catch (Error e) {
			// TODO: handle exception
			getErrors().add(e);
			AssertFailedLog();
			getAssertInfolList().add(verityStr+":failed");
	...
	public  static void VerityCationString(String actual,String exceptStr,String Message)
	{
		...
		try {
			Assert.assertTrue(flagBoolean);
			AssertPassLog();
			getAssertInfolList().add(Message+verityStr+":pass");
			getMessageList().add(Message+":pass");

		} catch (Error e) {
			// TODO: handle exception
			AssertFailedLog();
			getErrors().add(e);
			getAssertInfolList().add(Message+verityStr+":failed");
			getMessageList().add(Message+":failed");
			Assertion.snapshotInfo();
		}
	...
	public static void VerityString(String actual,String exceptStr)
	{
		...
		try {
			...
			// TODO: handle exception
			AssertFailedLog();
			RPAThreadLocal.addErrorIndex(1);
			getErrors().add(e);
			getAssertInfolList().add(verityStr+":failed");
			Assertion.snapshotInfo();
			//e.printStackTrace();
	...
	public static  void VerityTextPresent(String exceptStr)
	{
		...
		try {
			...
			getWebDriver().findElements(By.xpath(exceptStr));
			if (getWebDriver().findElements(By.xpath(exceptStr)).size()>0) {
				flag=true;
			}
			else {
				flag=false;
			...
			flag=false;
			ElementAction.getNoSuchElementExceptions().add(e);
			e.printStackTrace();
		}
		try {
	...
	public static  void VerityTextPresent(String exceptStr,String Message)
	{
		...
		try {
			...
			List<WebElement> webElements= getWebDriver().findElements(By.xpath(exceptStr));
	...
	public static  void VerityNotTextPresent(String exceptStr)
	{
		...
		try {
			...
			getWebDriver().findElement(By.xpath(exceptStr));
			flag=false;
		} catch (NoSuchElementException e) {
			// TODO: handle exception
			flag=true;
	...
	public static  void VerityTextPresentPrecision(String exceptStr,String Message)
	{
		...
		try {
			exceptStr="//*[text()=\""+exceptStr+"\"]";
			WebElement webElement=getWebDriver().findElement(By.xpath(exceptStr));
			System.out.println(exceptStr);
			flag=true;
		} catch (NoSuchElementException e) {
			// TODO: handle exception
	...
	{
		Assert.assertEquals(getErrors().size(), 0);
		//有找不到元素的异常也认为用例失败
		Assert.assertEquals(ElementAction.getNoSuchElementExceptions().size(), 0);
	}

	public static List<Error> getErrors() {
		return RPAThreadLocal.getErrors();
	}

	public static List<String> getAssertInfolList() {
		return RPAThreadLocal.getAssertInfolList();
	}

	public static List<String> getMessageList() {
		return RPAThreadLocal.getMessageList();
	}
