...
public class TestListener  extends TestListenerAdapter{
	...
	public void onTestStart(ITestResult  tr)
	{
		//tr.getTestContext()
		//方法开始前初始化报表描述信息
		Assertion.getErrors().clear();
		RPAThreadLocal.setErrorIndex(0);
		Assertion.getMessageList().clear();
		ElementAction.getNoSuchElementExceptions().clear();
	...
	public void onTestFailure( ITestResult  tr)
	{
		...
		//设置截图名字
		screenShot.setscreenName(tr.getMethod().getDescription()+RPAThreadLocal.getErrorIndex().toString());
		log.error(RPAThreadLocal.getErrorIndex().toString());
		screenShot.takeScreenshot();
		for(int i=0;i<Assertion.getMessageList().size();i++)
		{
			...
			if(Assertion.getMessageList().get(i).contains("pass"))
			{
				sb.append("<span class=\"pass_span\">"+Assertion.getMessageList().get(i)+"</span></br>\n");
			}
			else if(Assertion.getMessageList().get(i).contains("failed"))
			{
				sb.append("<span class=\"err_span\">"+Assertion.getMessageList().get(i)+"</span></br>\n");
			}
			else if(Assertion.getMessageList().get(i).contains("点击查看大图"))
			{
				sb.append("<span class=\"err_span\">"+Assertion.getMessageList().get(i)+"</span>\n");
			}
			sb.append("</err_assert_info>\n");
		}


		for (Exception e : ElementAction.getNoSuchElementExceptions()) {
		...
		for (Error e : Assertion.getErrors()) {
			StackTraceElement[] errorTraces = e.getStackTrace();
			StackTraceElement[] et = this.getKeyStackTrace(tr, errorTraces);
			if (tr.getParameters().length>0) {
				sb.append("<err_assert_info_StackTrace"+"   method=\""+tr.getTestClass().getName()+"."+tr.getMethod().getMethodName()+"."+tr.getEndMillis()+"\">\n");
	...
	public void onTestSuccess(ITestResult tr) {
		for(int i=0;i<Assertion.getMessageList().size();i++)
		{
			...
			sb2.append("<span class=\"pass_span\">"+Assertion.getMessageList().get(i)+"</span></br>\n");
